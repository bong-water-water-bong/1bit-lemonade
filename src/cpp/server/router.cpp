...
std::unique_ptr<WrappedServer> Router::create_backend_server(const ModelInfo& model_info) {
    ...
    } else if (model_info.recipe == "bitnet") {
        LOG(DEBUG, "Router") << "Creating BitNet (rocm-cpp) backend" << std::endl;
        new_server = std::make_unique<backends::BitNetServer>(log_level, model_manager_, backend_manager_);
    } else {
